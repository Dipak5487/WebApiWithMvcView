...
namespace MvcApp.Clinets
{
    public class UserClient : IUserClient
    {
        ...
        public async Task<SearchUserResponseDto?> Search(SearchUserDto searchUserDto)
        {
            var httpCleint = _httpClientFactory.CreateClient();
            var response = await httpCleint.PostAsync(new Uri($"{_configuration.GetValue<string>("UserApiUrl")}search"),
                new StringContent(JsonConvert.SerializeObject(searchUserDto)));
            if (!response.IsSuccessStatusCode)
            {
                return null;
            }
        ...
        {
            var httpCleint = _httpClientFactory.CreateClient();
            var response = await httpCleint.PostAsync(new Uri($"{_configuration.GetValue<string>("UserApiUrl")}search/"+id),
